      ...
      if(RequestURL.equals("/LoginServlet")){
			...
			if(AccountType.equals("Checking"))
						{
			                ...
			                String time = (h + ":" + m + ":" + s);
							Transaction tr=new Transaction(date,Float.parseFloat(OpeningDeposit), "Deposit", time, "", AccountNumber, Username);
		...
		if(account.equals("x"))
			...
			try {
				...
				if (accType.equals("s")) {
				SavingsAccount sav = new SavingsAccount(account.substring(1), CustomerName, userID, Float.parseFloat(amount));
				Boolean resultdoneSav = sav.Withdraw(sav.getSANum(), Float.parseFloat(amount));

				if (resultdoneSav) {
				output.println("<br><br><b>Successfully Withdraw Amount from From Savings Account!</center></b>");
				Date date = new Date();
				String h = String.format("%02d", date.getHours());
				String m = String.format("%02d", date.getMinutes());
				String s = String.format("%02d", date.getSeconds());
				String time = (h + ":" + m + ":" + s);
				Transaction transct = new Transaction(new Date(), Float.parseFloat(amount), "Withdraw", time, account.substring(1), null,userID);
				transct.recordTransaction();
				} else {
                output.println("<br><br><b><center>Unsuccessfully Withdraw Amount from From Savings Account!</center></b>");
		           }
		        } else  if(accType.equals("c")) {
				CheckingAccount check = new CheckingAccount(account.substring(1), CustomerName, userID, Float.parseFloat(amount));
		        Boolean resultdonecheck = check.deposit(check.getCANum(), Float.parseFloat(amount));
                if (resultdonecheck)
		        output.println("<br><br><b><center>Successfully Withdraw Amount from To Checking Account!</center></b>");
		        Date date = new Date();
		        String h = String.format("%02d", date.getHours());
		        String m = String.format("%02d", date.getMinutes());
		        String s = String.format("%02d", date.getSeconds());
		        ...
		        transct.recordTransaction();
			      ...
			      output.println("<br><br><b><center>UnSuccessfully Withdraw Amount from To Checking Account!</center></b>");
	         }
						...
						if (resultdoneSav) {
						output.println("<br><br><b><center>Successfully Deposit Amount from To Savings Account!</center></b>");
						Date date = new Date();
						String h = String.format("%02d", date.getHours());
						String m = String.format("%02d", date.getMinutes());
						String s = String.format("%02d", date.getSeconds());
						String time = (h + ":" + m + ":" + s);
						Transaction transct = new Transaction(new Date(), Float.parseFloat(amount), "Deposit", time, null, account.substring(1),userID);
						transct.recordTransaction();
						} else {
			                     output.println("<br><br><b><center>Unsuccessfully Deposit Amount from To Savings Account!</center></b>");
			                }
			            } else  if(accType.equals("c")) {
							CheckingAccount check = new CheckingAccount(account.substring(1), CustomerName, userID, Float.parseFloat(amount));
			                Boolean resultdonecheck = check.deposit(check.getCANum(), Float.parseFloat(amount));
                        if (resultdonecheck)
			                    output.println("<br><br><b><center>Successfully Deposit Amount from To Checking Account!</center></b>");
			                    Date date = new Date();
			                    String h = String.format("%02d", date.getHours());
			                    String m = String.format("%02d", date.getMinutes());
			                    String s = String.format("%02d", date.getSeconds());
			                    ...
			                    output.println("<br><br><b><center>UnSuccessfully Deposit Amount from To Checking Account!</center></b>");
			                }
		   ...
		   if(toAccount.equals("x") || fromAccount.equals("x")) {
			...
			if(fromType.equals("c") && toType.equals("c")){
			            CheckingAccount check = new CheckingAccount(fromAccount.substring(1), CustomerName, userID, Float.parseFloat(amount));
			            Boolean withdrawstatus = check.Withdraw(check.getCANum(), Float.parseFloat(amount));
			            Boolean depositstatus = false;
			            if(withdrawstatus)
			            {
			                CheckingAccount check2 = new CheckingAccount(toAccount.substring(1), CustomerName, userID, Float.parseFloat(amount));
			            ...
			            if (withdrawstatus && depositstatus) {
			                ...
			                String s = String.format("%02d", date.getSeconds());
			                String time = (h + ":" + m + ":" + s);
			                Transaction transct = new Transaction(new Date(), Float.parseFloat(amount), "Transfer", time, fromAccount.substring(1),toAccount.substring(1), userID);
			               	transct.recordTransaction();
			            } else {
			...
			if(fromType.equals("s") && toType.equals("s")){
			            SavingsAccount sav = new SavingsAccount(fromAccount.substring(1), CustomerName, userID, Float.parseFloat(amount));
			            Boolean withdrawstatus = sav.Withdraw(sav.getSANum(), Float.parseFloat(amount));
			            Boolean depositstatus = false;
			            if(withdrawstatus)
			            {
			                SavingsAccount sav2 = new SavingsAccount(toAccount.substring(1), CustomerName, userID, Float.parseFloat(amount));
			...
			if(fromType.equals("c") && toType.equals("s")){
			            ...
			            if(withdrawstatus)
			            {
			                SavingsAccount sav = new SavingsAccount(toAccount.substring(1), CustomerName, userID, Float.parseFloat(amount));
			       ...
			       if(fromType.equals("s") && toType.equals("c")){
			            ...
			            Boolean depositstatus = false;
			            if(withdrawstatus){
			                CheckingAccount check = new CheckingAccount(toAccount.substring(1), CustomerName, userID, Float.parseFloat(amount));
			...
			Vector [] arr = user.Inquire(str2, str1);
			Vector columnNames = arr[0];
			Vector data = arr[1];
			if(data.isEmpty())
			{
				output.println("<br><br><b><center>No Transactions available!</center></b>");
				return;
			}
